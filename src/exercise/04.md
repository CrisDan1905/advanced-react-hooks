...
Basically, the difference between `useLayoutEffect` and `useEffect` is that `useLayoutEffect` runs after 
the rendering but before the painting of the browser. `useEffect` is called after both of them happen,
so `useLaoyoutEffect` is executed before `useEffect`.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=04%3A%20useLayoutEffect%3A%20auto-scrolling%20textarea&em=danilo1905%40outlook.com).
